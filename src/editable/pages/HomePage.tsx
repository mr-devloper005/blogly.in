...
export default async function HomePage() {
  // Public surface is centred on the Reference Library (pdf task).
  const pdfEnabled = SITE_CONFIG.tasks.find((task) => task.key === 'pdf' && task.enabled)
  const primaryTask = (pdfEnabled?.key || SITE_CONFIG.tasks.find((task) => task.enabled)?.key || 'article') as TaskKey
  ...
  return (
    <EditableSiteShell>
      <main>
        ...
        <div className="mx-auto max-w-[var(--editable-container)] px-6 sm:px-8 lg:px-12">
